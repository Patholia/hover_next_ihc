...
def validation(
    ...
    for batch in tqdm(validation_dataloader):
        ...
        if len(batch) == 3:
            raw, gt_b, marker_id = batch
            marker_id = marker_id.to(device)
        else:
            raw, gt_b = batch
            marker_id = None
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            with torch.no_grad():
                out_ = model(raw, marker_id=marker_id)
            pred_inst = out_[:, :5]
            pred_class = out_[:, 5:]
            gt_inst = gt_b[:, 0]
            gt_ct = gt_b[:, 1]
    ...
    if metric == "lizard":
        ...
        # DÜZELTME: önceden bu print'ler ham torch tensor'ü basıyordu
        # ("PQ: tensor(0.4523, device='cuda:0')"), bu da eğitim log dosyasını
        # regex ile parse edip grafik üreten notebook hücresinin hiçbir sayısal
        # değeri yakalayamamasına yol açıyordu (sonuç: "236 validation record"
        # parse edildi ama 0 grafik üretildi). .item() ile düz float basılıyor.
        print("PQ:", pq_p.item())
        print("mPQ+", mpq_p.item())
        print("Validation loss: ", val_new.item())
        return mpq_p
    elif metric == "pannuke":
        ...
        print("Pannuke mPQ:", pan_mpq.item())
        print("Validation loss: ", val_new.item())
        return pan_mpq
    else:
        ...
        print("F1:", f1_c.mean().item())
        print("HD:", hd_c.mean().item())
        print("Validation loss: ", val_new.item())

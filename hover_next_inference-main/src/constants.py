...
# DÜZELTME: bu liste önceden orijinal PanNuke sınıflarına (neo/inf/con/dead/epi) göreydi.
# Bizim custom IHC checkpoint'imizin sınıf sırası: positive, negative, background, dead, stromal.
COLORS_PANNUKE = [
    [255, 0, 0],    # positive (kırmızı — DAB pozitif)
    [0, 127, 255],  # negative (mavi — boyasız)
...
# DÜZELTME: önceden orijinal PanNuke sınıf adlarıydı (neoplastic/inflammatory/...).
# Custom IHC checkpoint'imizin sınıf sırası farklı; geojson/export etiketleri
# artık gerçek sınıf isimleriyle eşleşiyor.

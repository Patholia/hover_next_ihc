...
def supervised_training(params):
    ...
    # DÜZELTME: önceden optimizer state dict yüklemesi try/except'siz idi —
    # dışarıdan indirilen (örn. Zenodo pretrained) bir checkpoint'te
    # optimizer_state_dict olmayabilir ya da farklı bir parametrizasyondan
    # gelebilir; bu durumda eskiden train.py tamamen çöküyordu.
    if "checkpoint_path" in params.keys() and params["checkpoint_path"]:
        try:
            _cp = torch.load(
                params["checkpoint_path"], map_location="cpu", weights_only=False
            )
            if "optimizer_state_dict" in _cp:
                optimizer.load_state_dict(_cp["optimizer_state_dict"])
                print("Load optimizer state dict")
            else:
                print("checkpoint'te optimizer_state_dict yok, optimizer sıfırdan başlıyor")
        except Exception as _e:
            print(f"Optimizer state dict yüklenemedi, sıfırdan başlıyor: {_e}")
    ...
    while step < params["training_steps"]:
        ...
        for _ in range(sz):
            ...
            batch = next(train_loaders[random.randint(0, len(train_loaders) - 1)])
            # IHCSliceDataset: (img, label, marker_id) | SliceDataset: (img, label)
            if len(batch) == 3:
            ...
            if step % (params["validation_step"] // world_size) == 0:
                ...
                if rank == 0:
                    ...
                    # DÜZELTME: önceden last_model her zaman kaydedilmiyordu —
                    # eğitim aksarsa (Colab kopması, OOM, vb.) en güncel ağırlık
                    # checkpoint_step periyodunu bekliyordu. Artık her validasyonda
                    # last_model de güncelleniyor, böylece resume her zaman en
                    # taze noktadan yapılabilir.
                    save_model(
                        step,
                        ddp_model,
                        optimizer,
                        loss,
                        val_new,
                        os.path.join(log_dir, "last_model"),
                    )
                ep_cnt += 1
                ddp_model.train()
                sys.stdout.flush()

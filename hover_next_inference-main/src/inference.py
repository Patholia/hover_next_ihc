    ...
    with ThreadPoolExecutor(max_workers=params["inf_writers"]) as executor:
        ...
        for raw, _ in tqdm(dataloader):
            ...
            with torch.inference_mode():
                # marker_id: inference sırasında --marker argümanından gelir
                marker_id = None
                if params.get("marker_id") is not None:
                    marker_id = torch.tensor(
                        [params["marker_id"]] * raw.shape[0], device=raw.device
                    )
...
def get_inference_setup(params):
    ...
    for pth in params["data_dirs"]:
        ...
        model = get_model(
            ...
            n_markers=mod_params.get("n_markers", 4),
        ).to(device)

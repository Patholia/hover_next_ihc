...
def _pannuke_to_conic(labels):
    """
    DÜZELTME: Bu dönüşüm önceden YALNIZCA notebook'larda çalışma-zamanı (runtime)
    yaması olarak ekleniyordu, repoda hiç yoktu.

    Sebep: UNIStainNet/MIST'ten gelen IHC label dosyaları (`labels.npy`) orijinal
    PanNuke maske formatında — [N, H, W, K] şeklinde, K-1 kanal her sınıf için
    binary one-hot maske + son kanal arka plan. `add_3c_gt_fast()` ise label
    array'inin [N, H, W, 2] formatında olmasını bekler: kanal 0 = instance id
    haritası, kanal 1 = sınıf id haritası.

    Bu dönüşüm yapılmadan `add_3c_gt_fast(lbls)` çağrıldığında, `Y[..., 0]`
    ("instance map" diye okunan kanal) gerçekte PanNuke'nin 1. sınıf binary
    maskesi oluyordu — gerçek hücre instance kimlikleri değil. Bu, modelin
    pozitif/negatif sınıflarını hiç doğru öğrenememesinin (collapse) önemli
    nedenlerinden biriydi. K<=3 ise veri zaten beklenen formatta, dokunulmaz.
    """
    if labels.ndim != 4 or labels.shape[-1] <= 3:
        return labels
    from scipy.ndimage import label as _cc_label

    combined = np.sum(labels[..., :-1], axis=-1).astype(np.int32)
    y_inst = np.stack([_cc_label(s)[0] for s in combined]).astype(np.float32)

    n_cls = labels.shape[-1] - 1
    y_cls = np.max(
        (labels[..., :-1] > 0) * np.arange(1, n_cls + 1, dtype=np.float32),
        axis=-1,
    )
    return np.stack([y_inst, y_cls], axis=-1)
...
def get_ihc_pannuke(params):
    ...
    for marker in markers:
        ...
        try:
            ...
            print(f"[UYARI] {e}")
            continue
        lbls = add_3c_gt_fast(_pannuke_to_conic(lbls))
        tr_imgs.append(imgs)
        tr_lbls.append(lbls)

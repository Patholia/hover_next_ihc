...
from src.data_utils import SliceDataset, IHCSliceDataset, _load_marker_data, add_3c_gt_fast
...
def main(nclasses, class_names, cp_paths, params, rank=0):
    ...
    if params["dataset"] == "ihc_pannuke":
        # DÜZELTME: evaluate.py önceden "ihc_pannuke" dataset tipini hiç tanımıyordu,
        # if/else sadece "pannuke" / "değilse lizard" diye ayırıyordu — yani IHC
        # checkpoint'i değerlendirilmeye çalışıldığında yanlışlıkla Lizard test verisini
        # (data_path_liz / data_path_mit) yüklemeye çalışıyor, bu da dosya bulunamadı
        # hatasına veya tamamen anlamsız bir değerlendirmeye yol açıyordu.
        ...
        for marker in markers:
            ...
            try:
                imgs, lbls = _load_marker_data(
                    params["data_path"], marker, split=val_split
                )
            except FileNotFoundError as e:
                print(f"[UYARI] {e}")
                continue
            va_imgs.append(imgs)
            va_lbls.append(add_3c_gt_fast(lbls))
        ...
        ds_list.append(IHCSliceDataset(x_val, y_val, marker_val))
        ds_names.append("ihc_val")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="DÜZELTME: alternatif olarak, eğitimde kullanılan config dosyasını "
        "(örn. sample_configs/train_ihc_pannuke.toml) doğrudan verebilirsiniz — "
        "deney klasörü train.py'deki aynı kurala göre (experiment + '_' + fold) "
    ...
    if args.config:
        ...
        args.exp = cfg["experiment"] + "_" + str(cfg["fold"])
    if not args.exp:
        raise ValueError("either --exp or --config must be given")
    ...
    if params["dataset"] == "pannuke":
        ...
        nclasses = 5
    elif params["dataset"] == "ihc_pannuke":
        class_names = CLASS_NAMES_IHC
        nclasses = 5
    else:
        class_names = CLASS_NAMES
        nclasses = 7
